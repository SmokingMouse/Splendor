...
class Splendor:
    ...
    def move(self, move):
        ...
        if action not in self.avaliable_actions:
            assert False, "invalid action, avaliable actions: " + str(
                self.avaliable_actions
            )

        if self.return_tokens and action != "return":
            assert False, (
                "invalid action, when you have more than 10 tokens you can only return them"
            )
        ...
        if action == "buy":
            ...
            assert False, "invalied action, avaliable actions: " + str(
                self.avaliable_actions
            )
    ...
    def do_return_tokens(self, requested_tokens):
        for color in requested_tokens:
            self.players[self.current_player]["tokens"][color] -= requested_tokens[
                color
            ]
    ...
    def show_cards(self):
        ...
        str_tier1 = [
            self.card_to_colors(shown_tier1.ix[i]) for i in range(len(shown_tier1))
        ]
        str_tier2 = [
            self.card_to_colors(shown_tier2.ix[i]) for i in range(len(shown_tier2))
        ]
        str_tier3 = [
            self.card_to_colors(shown_tier3.ix[i]) for i in range(len(shown_tier3))
        ]
    ...
    def can_afford(self, card):
        ...
        if not any(
            self.card_to_colors(card) in i for i in str_cards + str_reservations
        ):
            return False
